...
precios <-mutate(df,Ano=NULL,Mes=NULL, Categoria=NULL, 'Sub-categoria'=NULL,Producto=NULL,Pais=NULL,Cadena=NULL,Marca_propia=NULL,Promocion=NULL,ano,mes,categoria,subcategoria,producto,pais,cadena,marca,marcapropia,promocion)
...
```
```{r}
#creamos un columna vacia que sera los precios en clp
clp<- c(1:13966)
#la pegamos al dataframe precios
precios<-mutate(precios,clp)
#pasamos a numerico clp
precios$clp <-as.numeric(clp)
#Vamos a convertir a peso chileno. 10 es la columna del país y 15 corresponde a
#precios en clp
# datos extraidos  de https://cuex.com/es/clp-pen el 27 de abril de 2022
for (x in 1:nrow(precios)) {
  if (precios[x,10] == "Argentina") {
        precios[x,15]= precios[x,3]*7.35
  }
  if (precios[x,10] == "Colombia") {
        precios[x,15]= precios[x,3]*0.21
  } 
  if (precios[x,10] == "España") {
        precios[x,15]= precios[x,3]*892.47
  }
  if (precios[x,10] == "Perú") {
        precios[x,15]= precios[x,3]*221.21
  }
  if (precios[x,10] == "Portugal") {
        precios[x,15]= precios[x,3]*892.47
  }
  if (precios[x,10] == "UK") {
        precios[x,15]= precios[x,3]*1060.6
  }
}
```
